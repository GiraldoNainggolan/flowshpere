...
import { User, Mail, Bell, Shield, Monitor, Camera, Check } from "lucide-react";
import { createClient } from "../../src/lib/supabase/client";
import type { User as SupabaseUser } from "@supabase/supabase-js";
import { toast } from "sonner"; // Pastikan kamu sudah menginstal sonner untuk notifikasi
...
export default function SettingsPage() {
  ...
  // --- STATE UNTUK INTERAKSI FORM ---
  const fileInputRef = useRef<HTMLInputElement>(null);
  const [isSaving, setIsSaving] = useState(false);
  
  // State lokal untuk menyimpan perubahan (Dummy Data sebelum dikirim ke DB)
  ...
  // 1. Fungsi saat tombol "Change" foto diklik
  const handlePhotoClick = () => {
    fileInputRef.current?.click();
  };

  // 2. Fungsi saat foto dipilih dari laptop
  const handlePhotoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      // Membuat URL sementara dari file lokal laptop untuk preview
      const imageUrl = URL.createObjectURL(file);
      setLocalAvatar(imageUrl);
      toast.success("Foto berhasil dimuat. Jangan lupa simpan perubahan!");
    }
  };

  // 3. Fungsi saat tombol "Remove" diklik
  const handleRemovePhoto = () => {
    setLocalAvatar(null);
    toast.info("Foto dihapus. Menggunakan inisial nama.");
  };

  // 4. Fungsi saat tombol "Save Changes" diklik
  const handleSaveChanges = (e: React.FormEvent) => {
    e.preventDefault();
    setIsSaving(true);
    
    // Simulasi proses penyimpanan ke Database (Delay 1.5 detik)
    setTimeout(() => {
      setIsSaving(false);
    ...
    <div className="max-w-5xl mx-auto w-full animate-in fade-in duration-500 pb-10">
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1">
              ...
              <div className="flex items-center gap-6 mb-8">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <button onClick={handlePhotoClick} className="px-4 py-2 bg-white border border-gray-300 rounded-lg text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-xs active:scale-95">Change</button>
                    <button onClick={handleRemovePhoto} className="px-4 py-2 bg-white text-sm font-medium text-red-600 hover:bg-red-50 rounded-lg transition-colors active:scale-95">Remove</button>
                  </div>
                </div>
              </div>
              ...
              <form onSubmit={handleSaveChanges} className="space-y-5">
                ...
                <div className="pt-4 flex justify-end">
                  <button 
                    type="submit" 
                    disabled={isSaving}
                    className="px-6 py-2.5 bg-brand-500 hover:bg-brand-600 text-white rounded-lg font-medium transition-all active:scale-[0.98] shadow-sm disabled:opacity-70 flex items-center gap-2 min-w-[140px] justify-center"
                  >
                    {isSaving ? (
          ...
          {/* ... Sisa Tab Appearance & Account tetap sama seperti kode sebelumnya ... */}
          {activeTab === "appearance" && (
            <div className="bg-white border border-gray-200 rounded-2xl p-6 md:p-8 shadow-sm animate-in fade-in slide-in-from-bottom-4 duration-300">
              <h2 className="text-xl font-bold text-gray-900 mb-2">Theme Preferences</h2>
              <p className="text-sm text-gray-500 mb-6">Choose how FlowSphere looks to you.</p>
          ...
          {activeTab === "account" && (
            <div className="bg-white border border-gray-200 rounded-2xl p-6 md:p-8 shadow-sm flex flex-col items-center justify-center text-center h-64 animate-in fade-in slide-in-from-bottom-4 duration-300">
              ...
              <p className="text-sm text-gray-500 max-w-sm">Account security module is currently under development.</p>
            </div>
          )}
        </div>
      </div>
