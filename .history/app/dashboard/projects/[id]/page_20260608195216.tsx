...
export default function ProjectsPage() {
  ...
  // --- FUNGSI CREATE YANG SUDAH DIOPTIMALKAN ---
  const handleCreateProject = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!userId) return toast.error("Sesi tidak valid, silakan login ulang.");
    if (!newTitle.trim()) return toast.error("Judul proyek tidak boleh kosong.");

    setIsCreating(true);
    const { error } = await supabase.from('projects').insert([
      { 
        title: newTitle.trim(), 
        description: newDesc.trim() || 'Tidak ada deskripsi proyek.', 
        status: 'Aktif',
        user_id: userId 
      }
    ]);
    ...
    if (error) {
      ...
      setNewTitle(""); // Bersihkan input
      setNewDesc("");
      await refreshProjectsList(); // Tampilkan proyek baru secara real-time
    }
    setIsCreating(false);
  };
            ...
            <form onSubmit={handleCreateProject} className="p-6">
              ...
              <div className="mt-8 flex justify-end gap-3">
                ...
                <button 
                  type="submit" 
                  disabled={isCreating}
                  className="px-6 py-2.5 rounded-xl font-medium text-white bg-brand-500 hover:bg-brand-600 active:scale-[0.98] transition-all disabled:opacity-70 flex items-center gap-2"
                >
