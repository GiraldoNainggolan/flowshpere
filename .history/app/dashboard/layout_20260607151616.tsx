...
const UserAvatar = ({ avatarUrl, initials }: { avatarUrl?: string, initials: string }) => (
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  // --- LOGIKA PENERAP DARK MODE GLOBAL ---
  useEffect(() => {
    const getUser = async () => {
    ...
    // Fungsi untuk membaca localStorage dan menerapkan class "dark" ke tag <html>
    const applyTheme = () => {
    ...
    // Dengarkan siaran dari halaman Settings
  ...
  return (
    // Penambahan transisi global agar pergantian warna terlihat mulus (transition-colors duration-300)
    <div className="flex h-screen bg-surface dark:bg-gray-900 overflow-hidden text-text-primary dark:text-gray-100 font-sans transition-colors duration-300">
      ...
      `}>
        <div className="h-16 flex items-center px-6 border-b border-border-soft dark:border-gray-800 shrink-0">
          ...
          <button className="ml-auto md:hidden text-text-muted dark:text-gray-400" onClick={() => setIsMobileMenuOpen(false)}>
            <X size={20} />
          </button>
        </div>

        <div className="flex-1 overflow-y-auto py-6 px-4">
          <p className="px-3 text-xs font-bold text-text-muted dark:text-gray-500 uppercase tracking-wider mb-3">Workspace</p>
        ...
        {/* Profil Bawah */}
        <div className="p-4 border-t border-border-soft dark:border-gray-800 bg-surface dark:bg-gray-900">
          <div className="flex items-center gap-3">
            <UserAvatar avatarUrl={avatarUrl} initials={initials} />
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-text-primary dark:text-gray-100 truncate">{userName}</p>
              <p className="text-xs text-text-secondary dark:text-gray-400 truncate">{userEmail}</p>
            </div>
          </div>
        </div>
      </aside>
              ...
              {isNotifOpen && (
                <div className="absolute right-0 mt-3 w-80 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-xl dark:shadow-2xl rounded-2xl p-4 z-50 animate-in fade-in slide-in-from-top-2 ring-1 ring-black/5 dark:ring-white/5">
                  <div className="flex justify-between items-center mb-3">
                    ...
                    <span className="text-xs font-medium text-brand-600 dark:text-brand-400 hover:text-brand-700 cursor-pointer">Mark all as read</span>
                  </div>
                  <div className="space-y-3">
                    <div className="p-3 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-100 dark:border-gray-700 flex gap-3 cursor-pointer hover:border-brand-300 dark:hover:border-brand-500/50 transition-colors">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Alex mentioned you in &quot;FlowSphere Dev Sprint&quot;</p>
                      </div>
                    </div>
                  </div>
                </div>
              ...
              {isProfileOpen && (
                <div className="absolute right-0 mt-3 w-56 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-xl dark:shadow-2xl rounded-2xl py-2 z-50 animate-in fade-in slide-in-from-top-2 ring-1 ring-black/5 dark:ring-white/5">
                  ...
                  <Link 
                    ...
                    className="w-full text-left px-4 py-2 text-sm text-gray-600 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white hover:bg-gray-50 dark:hover:bg-gray-800 flex items-center gap-2 transition-colors"
                  >
                  ...
                  <button 
                    ...
                    className="w-full text-left px-4 py-2 text-sm text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-500/10 flex items-center gap-2 transition-colors font-medium"
                  >
